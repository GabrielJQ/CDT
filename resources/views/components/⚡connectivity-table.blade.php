...
new class extends Component
{
    ...
    {
        if (in_array($property, ['almacen', 'telefono', 'senial', 'compania', 'internet', 'tiendaSalud', 'perPage'], true)) {
            $this->page = 1;
        }
    }
    ...
    {
        return url('/conectividad?'.http_build_query(array_filter([
            'almacen' => trim($this->almacen),
            'telefono' => $this->telefono,
            'senial' => $this->senial,
            'compania' => $this->compania,
...
<div class="page-shell" wire:loading.class="opacity-70" wire:target="almacen,telefono,senial,compania,internet,sortBy,clearFilters,previousTablePage,nextTablePage,goToTablePage,showConnectivity">
    <div class="institutional-card mb-6 flex flex-col gap-4 border-l-4 border-[#988256] p-5 lg:flex-row lg:items-center lg:justify-between">
        <div>
            ...
            <p class="mt-1 max-w-3xl text-sm text-gray-500 dark:text-gray-400">Consulta disponibilidad de telefonía fija, señal celular, compañía e internet por tienda. Los filtros, KPIs y paginación se actualizan sin recargar la página.</p>
        </div>
