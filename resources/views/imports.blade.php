    ...
    <div class="page-shell">
        <section class="page-hero">
            <div class="page-hero-content">
                <div>
                    ...
                    <p class="page-subheading">Sube archivos de Tiendas Bienestar y Tiendas de Salud. Solo elige el archivo
                        correcto y subelo.</p>
                </div>
        ...
        <div class="grid grid-cols-1 gap-4 xl:grid-cols-2">
            <div class="xl:col-span-2 grid grid-cols-1 gap-4 md:grid-cols-2">
                @foreach(['regular' => 'Tiendas Bienestar', 'casa_x_casa' => 'Tiendas de Salud CxC'] as $tipo => $label)
                    ...
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 border-l-4 border-[#988256]">
                        ...
                        @if($periodoActivo)
                            <p class="mt-2 text-2xl font-bold text-green-700 dark:text-green-300">{{ $periodoActivo->anio }}
                                {{ $periodoActivo->trimestre }}</p>
                            <p class="text-xs text-gray-500 dark:text-gray-400">Corte:
                                {{ $periodoActivo->fecha_corte ? \Carbon\Carbon::parse($periodoActivo->fecha_corte)->format('d/m/Y') : 'Sin corte' }}
                                · {{ number_format($periodoActivo->total_filas) }} filas</p>
                        @else
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 lg:p-6 mb-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base lg:text-lg font-bold text-gray-800 dark:text-gray-100">📤 Subir archivo CSV —
                        Directorio Nacional De Tiendas</h3>
                ...
                <form action="{{ route('imports.upload') }}" method="POST" enctype="multipart/form-data" id="upload-form"
                    ...
                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-3">
                        <div>
                            ...
                            <input type="number" name="anio" value="{{ old('anio', $currentYear) }}" min="2020" max="2100"
                                required class="input-filter">
                        </div>
                        <div>
                        ...
                        <div>
                            <label class="block text-xs text-gray-500 dark:text-gray-400 uppercase mb-1">Fecha de
                                corte</label>
                            <input type="date" name="fecha_corte" value="{{ old('fecha_corte', '2026-04-01') }}"
                                class="input-filter">
                        </div>
                    </div>

                    <label
                        class="flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-xs text-amber-800 dark:border-amber-700 dark:bg-amber-900/20 dark:text-amber-200">
                    ...
                    <label for="csv_file"
                        ...
                        <div class="text-center">
                            ...
                            <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Haz clic para seleccionar un
                                archivo CSV
                            </p>
                    ...
                    <button id="submit-btn" type="submit" disabled class="btn-upload">
                        ⬆ Subir e Importar
                    </button>
                </form>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 rounded-xl shadow p-4 lg:p-6 mb-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-base lg:text-lg font-bold text-gray-800 dark:text-gray-100">📤 Subir Excel — Directorio Nacional de Tiendas Salud Casa x Casa
                        </h3>

                </div>
                <form action="{{ route('imports.upload-casa-x-casa') }}" method="POST" enctype="multipart/form-data"
                    id="cxc-upload-form" class="space-y-4">
                    @csrf
                    ...
                    <label for="xlsx_file"
                        ...
                        <div class="text-center">
                            <div class="text-3xl mb-2">📊</div>
                            <p class="text-sm font-medium text-gray-600 dark:text-gray-400">Haz clic para seleccionar
                                archivo Excel
                            </p>
                    ...
                    <button id="cxc-submit-btn" type="submit" disabled class="btn-upload">
                        ⬆ Subir e Importar
                    </button>
                </form>
            </div>
