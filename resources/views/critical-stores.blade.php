...
@section('content')
    ...
    <div id="app">
        ...
        <div class="bg-white rounded-xl shadow p-4 mb-4">
            <form method="GET" action="/informacion-tiendas" class="flex flex-wrap items-end gap-3">
                ...
                <div class="flex gap-2">
                    ...
                    <a href="/tiendas-criticas" class="bg-gray-200 hover:bg-gray-300 text-gray-700 px-4 py-2 rounded-lg text-sm font-semibold transition inline-block">Limpiar</a>
                </div>
            </form>
        </div>
        ...
        <div class="bg-white rounded-xl shadow overflow-x-auto">
            <table id="cs-table" class="w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr id="cs-header"></tr>
                </thead>
                <tbody id="cs-body" class="divide-y divide-gray-200"></tbody>
            </table>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        function renderCell(col, store) {
            ...
            if (col === 'Detalle') {
                if (!e.conditions || !e.labels) return '<span class="text-gray-400 text-xs">Sin incidencias</span>';
                var active = FACTOR_KEYS.filter(function (k) { return e.conditions[k]; });
                if (active.length === 0) return '<span class="text-gray-400 text-xs">Sin incidencias</span>';
                var factorStyles = {
                    capital_bajo: ['bg-purple-100 text-purple-800 border-purple-300', '💰'],
                    comite_vencido: ['bg-red-100 text-red-800 border-red-300', '📅'],
                    auditoria_elevada: ['bg-orange-100 text-orange-800 border-orange-300', '🔍'],
                    pagare_proximo: ['bg-blue-100 text-blue-800 border-blue-300', '📄'],
                    rotacion_baja: ['bg-amber-100 text-amber-800 border-amber-300', '📉'],
                    asamblea_pendiente: ['bg-cyan-100 text-cyan-800 border-cyan-300', '🗳️'],
                };
                return '<div class="flex flex-wrap gap-1.5 max-w-md">' +
                    active.map(function (k) {
                        var info = e.labels[k] || {};
                        var st = factorStyles[k] || ['bg-gray-100 text-gray-700 border-gray-300', '▪'];
        ...
        function renderTable() {
            ...
            body.innerHTML = pageData.map(function (store) {
                ...
                var bgClass = level === 'rojo' ? ' bg-red-50' : (level === 'amarillo' ? ' bg-amber-50' : '');
                return '<tr class="hover:bg-gray-100' + bgClass + '">' +
                    cols.map(function (c) {
